...
1. The main switch board looks like this: ![entry1](images/entry1.png). Put your cursor in the `PEGID` field, press `ctrl + F` to search for the participant ID that you are going to enter data for. Then, navigate to the corresponding form by clicking the button on the switch board. The format and layout of each form in the database should be consistent with the paper/eletronic version that we use in the interview. If you notice any discrepancy, please report to the data manager.
...
We also provide a data entry tracking report to monitor the data entry process. You can visualize the data entry progress through this link: [Data entry tracking](documents/peg3_data_entry.html). The report is updated every week and you can see the data entry status for the priority forms. You can also generate the ID list for the participants who have completed a specific instrument but not entered into the database yet.